...
func (fa *FileAnalyzer) Analyze() error {
	...
	// increase the max token size to handle very long lines (default is 64K)
...
func (fa *FileAnalyzer) finalizeWordFreq() {
	...
	// release the map to save memory
	fa.WordCounts = nil
}
...
func printSummary(analyzers []*FileAnalyzer, config *AnalysisConfig) {
	...
	// Build TOTAL row with the same number of columns as headers
	totalRow := make([]string, len(headers))
	totalRow[0] = "TOTAL"
	if len(totalRow) > 1 {
		totalRow[1] = strconv.Itoa(totals[0])
	}
	if len(totalRow) > 2 {
		totalRow[2] = strconv.Itoa(totals[1])
	}
	if len(totalRow) > 3 {
		totalRow[3] = strconv.Itoa(totals[2])
	}
	if len(totalRow) > 4 {
		totalRow[4] = strconv.Itoa(totals[3])
	}
	// longest line, encoding, top words (if present) stay empty

	for _, row := range table {
		printRow(row, widths)
	}
	printRow(totalRow, widths)
}
...
func saveResults(analyzers []*FileAnalyzer, config *AnalysisConfig) {
	...
	case "json":
		// Encode a lightweight result representation to avoid serializing internal runtime fields
		type outItem struct {
			FilePath string      `json:"file"`
			Results  FileResults `json:"results"`
			Error    string      `json:"error,omitempty"`
		}
		out := make([]outItem, 0, len(analyzers))
